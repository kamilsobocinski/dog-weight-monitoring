...
export function HistoryScreen({ dog, weights, onDelete }) {
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.text(`${t('setup.birthdate')}: ${dobStr}  |  ${t('setup.sex')}: ${sexStr}  |  Age: ${dogAgeStr(dog.birthdate)}`, margin + 4, 56)
      ...
      if (chartRef.current) {
        try {
          ...
          doc.addImage(imgData, 'PNG', margin, chartY, chartW, chartH)
          chartY += chartH + 8
        } catch (e) {
          chartY += 5
        }
      ...
      const rows = sorted.map((w, i) => {
        ...
        let dev = ''
        if (idealRange) {
          const d = w.value - ((idealRange.min + idealRange.max) / 2)
      ...
      autoTable(doc, {
        startY: chartY + 2,
        head: [['Date', 'Weight', 'Deviation', 'Breed norm', 'Note']],
        body: rows,
      ...
      for (let p = 1; p <= pages; p++) {
        ...
        doc.text(`Dog Weight Monitoring · Page ${p}/${pages}`, pageW / 2, doc.internal.pageSize.getHeight() - 8, { align: 'center' })
      }
      ...
      <div ref={chartRef} style={{ background: 'white', padding: '8px 4px', marginBottom: 12 }}>
        <ResponsiveContainer width="100%" height={180}>
          <LineChart data={chartData} margin={{ top: 4, right: 16, left: -16, bottom: 0 }}>
            ...
            {idealRange && <ReferenceLine y={idealRange.min} stroke="#16a34a" strokeDasharray="4 2" strokeWidth={1.5} />}
            {idealRange && <ReferenceLine y={idealRange.max} stroke="#dc2626" strokeDasharray="4 2" strokeWidth={1.5} />}
                ...
                <div className="weight-row">
                  ...
                  <div>
                    ...
                    {idealRange && (() => {
                      ...
                      const d = w.value - mid
                      return <div style={{ fontSize: 11, color: 'var(--gray-400)', marginTop: 2 }}>
